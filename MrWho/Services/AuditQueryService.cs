...
public sealed class AuditQueryService : IAuditQueryService
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (fromUtc.HasValue) q = q.Where(e => e.TimestampUtc >= fromUtc.Value);
        if (toUtc.HasValue) q = q.Where(e => e.TimestampUtc <= toUtc.Value);
        ...
        if (!string.IsNullOrWhiteSpace(actorUserId)) q = q.Where(e => e.ActorUserId == actorUserId);
        if (!string.IsNullOrWhiteSpace(actorClientId)) q = q.Where(e => e.ActorClientId == actorClientId);
