...
public static class DynamicClientRegistrationValidation
{
    ...
    {
        if (req == null) return (false, "invalid_request");
        // Require authorization_code or client_credentials at least one
        var grants = req.GrantTypes ?? new List<string>();
        if (grants.Count == 0) return (false, "At least one grant_type required");
        if (grants.Any(g => !SupportedGrantTypes.Contains(g))) return (false, "Unsupported grant_type");
        if (grants.Contains("authorization_code", StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (responses.Any(r => !SupportedResponseTypes.Contains(r))) return (false, "Unsupported response_type");
        }
        if (req.GrantTypes?.Contains("password", StringComparer.OrdinalIgnoreCase) == true) return (false, "password grant not allowed");
